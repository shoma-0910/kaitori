...

{
  ...
  // 商圏スコア計算エンドポイント
  app.post("/api/registered-stores/:id/calculate-market-power", requireAuth, async (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      ...
      const municipalityMatch = address.match(/(.+?[都道府県])(.+?[市区町村])/);
      const municipality = municipalityMatch ? municipalityMatch[2] : null;
      const prefecture = municipalityMatch ? municipalityMatch[1] : null;
      const region = prefecture && municipality ? `${prefecture}${municipality}` : null;

      let populationData: any = null;
      let seniorFemalePopulation = 0;
      let averageIncome = 0;
      ...
      if (region) {
        try {
          ...
          const demographics = await eStatClient.getRegionDemographics(region);
          
          if (demographics.population?.value) {
            populationData = {
              total: demographics.population.value,
              ageDistribution: demographics.ageDistribution?.value,
              genderRatio: demographics.genderRatio?.value,
            };
            
            // 60歳以上女性人口を推計（65歳以上人口 × 女性比率 × 1.2で60-64歳を加算）
            const population = demographics.population.value;
            const seniorPercentage = demographics.ageDistribution?.value?.find(a => a.range === "65歳以上")?.percentage || 0;
            const femaleRatio = demographics.genderRatio?.value?.female || 50;
            
            seniorFemalePopulation = Math.round(population * (seniorPercentage / 100) * (femaleRatio / 100) * 1.2);
          }
        } catch (error) {
          console.warn("Failed to fetch e-Stat data:", error);
        }
      }

      // Gemini AIで平均年収を推計（オプション）
      const geminiApiKey = process.env.GEMINI_API_KEY;
      if (geminiApiKey && region) {
        try {
          const { GoogleGenAI } = await import("@google/genai");
          const client = new GoogleGenAI({ apiKey: geminiApiKey });
          
          const prompt = `${region}の平均世帯年収（万円単位）を推定してください。
日本の統計データに基づいて、この地域の経済状況を考慮して推定してください。
回答は数値のみ（例: 450）で答えてください。`;
          ...
          const responseText = result.text || "";
          const incomeMatch = responseText.match(/\d+/);
          if (incomeMatch) {
            averageIncome = parseInt(incomeMatch[0]);
          }
          ...
          await storage.logApiUsage({
            ...
            apiType: "gemini",
            endpoint: "generateContent",
            metadata: JSON.stringify({ model: "gemini-1.5-flash", purpose: "income_estimation", region })
          });
        } catch (error) {
          console.warn("Failed to estimate income with Gemini:", error);
        }
      }

      // 商圏スコアを計算
      const { calculateMarketPower } = await import("./services/marketPowerCalculator");
      
      const marketPowerResult = calculateMarketPower({
        storeArchetype: store.storeArchetype as any,
        parkingSize: store.parkingSize as any,
        parkingCapacity: store.parkingCapacity,
        population1km: populationData?.total ? Math.round(populationData.total * 0.1) : 0, // 1km圏は市区町村人口の約10%
        population2km: populationData?.total ? Math.round(populationData.total * 0.25) : 0, // 2km圏は市区町村人口の約25%
        seniorFemalePopulation,
        averageIncome,
      });

      // 結果を保存
      await storage.updateRegisteredStore(id, req.organizationId!, {
        marketPowerScore: marketPowerResult.score,
      });

      res.json({
        score: marketPowerResult.score,
        breakdown: marketPowerResult.breakdown,
        region,
        populationData,
        seniorFemalePopulation,
        averageIncome,
        storeArchetype: store.storeArchetype,
        parkingSize: store.parkingSize,
      });

    } catch (error: any) {
      console.error("[POST /api/registered-stores/:id/calculate-market-power] Error:", error);
      res.status(500).json({ error: error.message });
    }
  });
